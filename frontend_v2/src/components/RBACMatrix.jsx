...
export default function RBACMatrix() {
    ...
    return (
        <div className="min-w-0 space-y-5 font-sans">
            ...
            <div className="order-last bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="order-first overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-2 border-b border-slate-100 p-6 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p className="text-[11px] font-black uppercase tracking-widest text-blue-500">Comptes existants</p>
                        <h2 className="mt-1 text-2xl font-black text-slate-900">Utilisateurs déjà créés</h2>
                    </div>
                    <span className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-black uppercase tracking-widest text-slate-500">
                    ...
                    <table className="w-full min-w-[900px] text-left">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Utilisateur</th>
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Profil principal</th>
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Accès</th>
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Contact</th>
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Stations</th>
                                <th className="px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500">Invitation</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {users.length > 0 ? users.map(existingUser => {
                                const preset = ROLE_PRESETS.find(item => item.name === existingUser.role) || ROLE_FALLBACKS[existingUser.role];
                                const displayName = `${existingUser.first_name || ''} ${existingUser.last_name || ''}`.trim() || existingUser.username;
                                return (
                                    <tr key={existingUser.id} className="hover:bg-slate-50/70">
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="truncate font-mono text-xs font-semibold text-slate-400">{existingUser.username}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4">
                                            <p className="text-sm font-black text-slate-800">{preset?.label || existingUser.role}</p>
                                            {existingUser.additional_roles?.length > 0 && (
                                                <p className="mt-1 text-xs font-bold text-blue-600">+ {existingUser.additional_roles.join(', ')}</p>
                                            )}
                                        </td>
                                        <td className="px-5 py-4">
                                            <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-black text-slate-600">
                                                {existingUser.access_mode || 'PIN'}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4 text-sm font-semibold text-slate-600">
                                            {existingUser.email || existingUser.phone ? (
                                                <>
                                                    {existingUser.email && <p>{existingUser.email}</p>}
                                                    {existingUser.phone && <p className="text-xs text-slate-400">{existingUser.phone}</p>}
                                                </>
                                            ) : (
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex max-w-xs flex-wrap gap-1.5">
                                                {existingUser.stations?.length > 0 ? existingUser.stations.map(station => (
                                                    <span key={station.id || station.code} className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-black text-slate-500">
                                                        {station.display_name || station.code}
                                                    </span>
                                                )) : (
                                                    <span className="text-xs font-semibold text-slate-300">Aucune station</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-2">
                                                <span className={`rounded-lg px-2.5 py-1 text-[11px] font-black ${
                                                    ['PENDING', 'INVITED'].includes(existingUser.invitation_status)
                                                        ? 'bg-amber-100 text-amber-700'
                                                        : 'bg-emerald-100 text-emerald-700'
                                                }`}>
                                                    {existingUser.invitation_status || 'ACTIVE'}
                                                </span>
                                                {existingUser.email && (
                                                    <button
                                                        type="button"
                                                        onClick={() => resendInvite(existingUser.id)}
                                                        className="rounded-lg border border-blue-100 bg-blue-50 px-2.5 py-1 text-[11px] font-black text-blue-700 hover:bg-blue-100"
                                                    >
                                                        Renvoyer
                                                    </button>
                                                )}
