...
const LOGISTICS_VIEWS = {
    ship: { label: 'À expédier', eyebrow: 'File opérationnelle', icon: Package },
    routes: { label: 'Tournées', eyebrow: 'Planification chauffeur', icon: Truck },
    notes: { label: 'Bons de livraison', eyebrow: 'Référentiel expédition', icon: FileText },
    driver: { label: 'Chauffeur', eyebrow: 'Exécution terrain', icon: MapPin },
    returns: { label: 'Retours & anomalies', eyebrow: 'Contrôle livraison', icon: AlertTriangle },
};
...
const WORKFLOW_STAGES = [
    { id: 'ready', title: 'Préparer', helper: 'BL prêt, adresse et contact exploitables.', icon: Package },
    { id: 'route', title: 'Planifier', helper: 'Affecter chauffeur, véhicule et date.', icon: Route },
    { id: 'transit', title: 'Livrer', helper: 'Démarrer la tournée puis suivre le terrain.', icon: Truck },
    { id: 'proof', title: 'Prouver', helper: 'Signature ou preuve attachée au BL.', icon: ShieldCheck },
    { id: 'issue', title: 'Contrôler', helper: 'Retour, litige ou livraison bloquée traité.', icon: AlertTriangle },
];
...
export default function DeliveryDashboard() {
    ...
    const activeQueueItems = queueItems.filter(item => item.next_action !== 'CLOSE');

    const searchMatches = value => normalizeText(value).includes(normalizeText(searchTerm));
    const filteredQueueItems = activeQueueItems.filter(item => {
        if (!searchTerm) return true;
        return searchMatches(item.reference)
            || searchMatches(item.client_name)
            || searchMatches(item.delivery_address)
            || searchMatches(item.status)
            || searchMatches(item.next_action);
    });
    const filteredNotes = allNotes.filter(note => {
        if (!searchTerm) return true;
        return searchMatches(note.reference)
            || searchMatches(note.client_name)
            || searchMatches(note.delivery_address)
            || searchMatches(note.status);
    });

    const readyCount = queueSummary.ready_count ?? readyNotes.length;
    const assignedCount = queueSummary.assigned_count ?? allNotes.filter(note => note.status === 'ASSIGNED').length;
    const inTransitCount = queueSummary.in_transit_count ?? allNotes.filter(note => note.status === 'IN_TRANSIT').length;
    ...
    const blockedCount = queueSummary.blocked_count || 0;
    const proofMissingCount = queueSummary.proof_missing_count || 0;

    const currentViewMeta = LOGISTICS_VIEWS[activeView] || LOGISTICS_VIEWS.ship;
    const CurrentIcon = currentViewMeta.icon;

    const changeView = view => setSearchParams({ view: 'logistics', logisticsMenu: view });
        ...
        const handleQueueAction = () => {
            if (item.next_action === 'ASSIGN_ROUTE') {
                setNewRouteData(prev => ({
                    ...prev,
                    note_ids: item.note_id ? Array.from(new Set([...prev.note_ids, item.note_id])) : prev.note_ids,
                }));
            ...
            <div key={item.note_id} className={`rounded-2xl border p-4 shadow-sm ${priorityClass}`}>
                <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                        ...
                        <h3 className="mt-2 truncate text-lg font-black text-slate-950">{item.client_name}</h3>
                    ...
                    <div className="rounded-xl border border-white/70 bg-white px-3 py-2 text-right shadow-sm">
    ...
    const renderNoteCard = (note, actionLabel = 'Planifier') => (
        <div key={note.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <h3 className="mt-1 truncate text-lg font-black text-slate-950">{note.client_name}</h3>
                    <p className="mt-1 flex items-start gap-2 text-sm font-bold text-slate-500">
                        <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" />
                ...
                <div className="flex flex-col gap-4 border-b border-slate-100 bg-slate-50 px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
                    <div>
                        ...
                        <p className="mt-2 flex flex-wrap gap-4 text-sm font-bold text-slate-500">
                            <span className="inline-flex items-center gap-1"><Calendar className="h-4 w-4" /> {route.planned_date ? new Date(route.planned_date).toLocaleDateString('fr-FR') : 'Date non renseignée'}</span>
    ...
    const renderMainContent = () => {
        if (isLoading) {
            return <div className="rounded-3xl border border-slate-200 bg-white py-20 text-center font-black text-slate-400">Chargement logistique...</div>;
        }
        if (activeView === 'routes') {
            return <div className="space-y-5">{routes.map(renderRouteCard)}{routes.length === 0 && <EmptyState icon={Truck} title="Aucune tournée" text="Créez une tournée depuis les BL prêts." />}</div>;
        }
        if (activeView === 'notes') {
            return <div className="grid grid-cols-1 gap-4 xl:grid-cols-2 2xl:grid-cols-3">{filteredNotes.map(note => renderNoteCard(note, 'Ouvrir'))}</div>;
        }
        if (activeView === 'driver') {
            const activeRoutes = routes.filter(route => ['PLANNED', 'IN_TRANSIT'].includes(route.status));
            return <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">{activeRoutes.map(renderRouteCard)}{activeRoutes.length === 0 && <EmptyState icon={Truck} title="Aucune tournée active" text="Les tournées planifiées ou en cours apparaîtront ici." />}</div>;
        }
        if (activeView === 'returns') {
            return <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">{issueNotes.map(note => renderNoteCard(note, 'Traiter'))}{issueNotes.length === 0 && <EmptyState icon={CheckCircle} title="Aucune anomalie logistique" text="Les retours, annulations et problèmes de livraison seront visibles ici." />}</div>;
        }
        return (
            <div className="grid grid-cols-1 gap-6 2xl:grid-cols-[1fr_380px]">
                <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p className="mt-1 text-sm font-bold text-slate-500">La priorité est calculée depuis statut BL, tournée, adresse, contact et preuve de livraison.</p>
                    <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
                        {filteredQueueItems.map(renderQueueItem)}
                        {filteredQueueItems.length === 0 && <EmptyState icon={Package} title="Aucune action logistique urgente" text="Les BL prêts, anomalies et tournées en cours apparaîtront ici." />}
                    </div>
                </section>
                <aside className="space-y-4">
                    <DecisionPanel
                        ...
                        proofMissingCount={proofMissingCount}
                    />
                    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Lecture rapide</p>
                        <div className="mt-4 space-y-3">
                            {[
                                ['BL à planifier', readyCount],
                                ['Tournées en cours', inTransitRoutes.length],
                                ['Preuves manquantes', proofMissingCount],
    ...
    return (
        <div className="min-h-[calc(100vh-96px)] bg-slate-50">
            <div className="border-y border-slate-200 bg-white px-6 py-6 xl:px-8">
                <div className="flex flex-col gap-5 2xl:flex-row 2xl:items-center 2xl:justify-between">
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-[0.35em] text-blue-500">{currentViewMeta.eyebrow}</p>
                        <div className="mt-2 flex items-center gap-3">
                            <CurrentIcon className="h-6 w-6 text-blue-600" />
                            <h2 className="text-3xl font-black text-slate-950">{currentViewMeta.label}</h2>
                        </div>
                        <p className="mt-1 text-sm font-bold text-slate-500">Préparer, planifier, livrer, prouver et traiter les écarts sans perdre le fil métier.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="mt-5 max-w-4xl">
                    <div className="relative">
                        ...
                        <input value={searchTerm} onChange={event => setSearchTerm(event.target.value)} placeholder="Rechercher BL, client, adresse, statut..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-12 pr-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-5 xl:px-8">
                <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Parcours logistique</p>
                            <h3 className="mt-1 text-xl font-black text-slate-950">BL prêt → tournée → livraison → preuve → contrôle</h3>
                        </div>
                        <p className="text-xs font-black uppercase tracking-widest text-slate-400">Sous-menus dans la sidebar</p>
                    </div>
                            ...
                            return (
                                <div key={stage.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                    <div className="flex items-center gap-3">
                                        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-sm font-black text-blue-600 shadow-sm">{index + 1}</span>
                                        <Icon className="h-5 w-5 text-slate-500" />
                                    </div>
                                    <h4 className="mt-4 text-base font-black text-slate-950">{stage.title}</h4>
                                    <p className="mt-1 text-xs font-bold leading-relaxed text-slate-500">{stage.helper}</p>
                                </div>
                            );
                        })}
                    </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-4 xl:grid-cols-6">
                    {[
                        ['À expédier', readyCount, 'BL prêts', 'amber'],
                        ['Assignés', assignedCount, 'À charger', 'blue'],
                        ['En tournée', inTransitCount, 'Terrain', 'indigo'],
                        ['Livrés', deliveredCount, 'Clôturés', 'emerald'],
                        ['Preuves manquantes', proofMissingCount, 'À archiver', 'slate'],
                        ['Anomalies', issueCount, 'À traiter', issueCount ? 'red' : 'slate'],
...
function DecisionPanel({ readyCount, plannedCount, issueCount, blockedCount, proofMissingCount }) {
    const decision = issueCount > 0
        ...
        : blockedCount > 0
            ...
            : readyCount > 0
                ...
                : proofMissingCount > 0
                    ? 'Archiver les preuves de livraison manquantes.'
                    : plannedCount > 0
                        ? 'Démarrer ou suivre les tournées planifiées.'
                        : 'Aucune action logistique urgente.';
    return (
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-[10px] font-black uppercase tracking-widest text-blue-500">Prochaine décision</p>
            ...
            <div className="mt-5 space-y-3 text-sm font-bold text-slate-600">
                ...
                <p className="flex gap-2"><Clock className="h-5 w-5 text-slate-400" /> Clôturer uniquement avec preuve ou anomalie qualifiée.</p>
            </div>
        </div>
    );
}
