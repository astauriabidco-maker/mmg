    ...
    const submitPlan = async event => {
        ...
        try {
            const payload = {
                ...
                status: planForm.assigned_user_id && planForm.scheduled_start ? 'SCHEDULED' : 'TO_SCHEDULE',
            };
            ...
            <main className="min-h-screen lg:ml-72">
                <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-slate-200 bg-white px-4 md:px-7">
                    ...
                    <span className="truncate text-sm font-black text-slate-900">
                        {isNew ? 'Planifier une mission de métré' : mission?.reference}
                    </span>
                </header>
                ...
                {isNew ? (
                    <form onSubmit={submitPlan}>
                        <section className="border-b border-slate-200 bg-white px-4 py-7 md:px-7">
                            ...
                            <h1 className="mt-1 text-2xl font-black text-slate-950 md:text-3xl">Planifier une mission de métré</h1>
                            <p className="mt-2 max-w-3xl text-sm font-semibold text-slate-500">
                                Le client, le chantier et le métreur restent visibles avant toute saisie technique.
                            </p>
                        </section>
                        <section className="grid gap-8 px-4 py-7 md:px-7 xl:grid-cols-[minmax(0,1fr)_360px]">
                            <div className="space-y-8">
                                <div>
                                    <h2 className="mb-4 text-lg font-black text-slate-900">1. Client et chantier</h2>
                                ...
                                <div className="border-t border-slate-200 pt-7">
                                    <h2 className="mb-4 text-lg font-black text-slate-900">2. Affectation et rendez-vous</h2>
                                    ...
                                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                                        <Field label="Objet de la mission"><input value={planForm.purpose} onChange={event => setPlanForm(current => ({ ...current, purpose: event.target.value }))} placeholder="Ex. remplacement de 6 menuiseries" className={inputClass} /></Field>
                            ...
                            <aside className="self-start border-l-0 border-slate-200 xl:border-l xl:pl-8">
                                <h2 className="text-lg font-black text-slate-900">Mission prête ?</h2>
                        ...
                        <section className="border-b border-slate-200 bg-white px-4 py-6 md:px-7">
                            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {mission.status === 'SCHEDULED' && <button onClick={() => changeStatus('ON_SITE')} className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-black text-white">Démarrer sur site</button>}
                                    {['ON_SITE', 'CORRECTION_REQUIRED'].includes(mission.status) && <button onClick={() => changeStatus('TO_REVIEW')} className="rounded-lg bg-orange-600 px-4 py-2.5 text-sm font-black text-white">Envoyer au contrôle BE</button>}
                            ...
                            <div className="mt-6 grid gap-4 border-t border-slate-100 pt-5 sm:grid-cols-2 xl:grid-cols-4">
                                ...
                                <div className="flex gap-3"><UserRound className="h-5 w-5 text-indigo-600" /><div><p className="text-[10px] font-black uppercase text-slate-400">Métreur</p><p className="text-sm font-black text-slate-800">{mission.assigned_user_name || 'Non affecté'}</p></div></div>
                                <div className="flex gap-3"><Clock3 className="h-5 w-5 text-amber-600" /><div><p className="text-[10px] font-black uppercase text-slate-400">Rendez-vous</p><p className="text-sm font-black text-slate-800">{formatDate(mission.scheduled_start)}</p></div></div>
                        ...
                        <section className="grid min-h-[calc(100vh-330px)] lg:grid-cols-[300px_minmax(0,1fr)]">
                            <aside className="border-b border-slate-200 bg-white lg:border-b-0 lg:border-r">
                                ...
                                <div className="max-h-72 overflow-y-auto p-2 lg:max-h-none">
                                    ...
                                    {!openingCount && <p className="p-6 text-center text-sm font-bold text-slate-400">Ajoutez le premier ouvrage relevé sur le chantier.</p>}
                                </div>
                            </aside>
                                    ...
                                    {(selectedOpeningId || !missionLocked) ? (
                                        <div className="py-6">
                                            ...
                                            <div className="my-6 border-t border-slate-200 pt-6">
                                                <div className="mb-4 flex items-center gap-2"><Ruler className="h-5 w-5 text-blue-600" /><h3 className="font-black text-slate-900">Dimensions relevées</h3></div>
